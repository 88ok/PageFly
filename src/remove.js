...
//
//   举报/删除历史表(PAGEDROP_DELREQ_KV，独立 KV)：key = 原始页面 id，value = 联系方式（单个）。
...
// env.PAGEDROP_DELREQ_KV 可选，提供 put（举报历史表，独立 KV）
//
// 历史表约定：key = 原始页面 id，value = 联系方式（单个）。
//   举报一次链接即失效、原 id 已变，第二个用户无法再拿原链接举报，故无需考虑多人累加。
